...
* Em testes, você normalmente nunca captura exceções, mas apenas as propaga, com `@Test void testXYZ() throws SomeException, AnotherException`..., para que o teste falhe se a exceção acontecer.  Unless you actually really want to test for the occurence of a problem - in that case, use [JUnit's Assert.assertThrows()](https://github.com/junit-team/junit4/wiki/Exception-testing) (but not `@Test(expected = SomeException.class)`).
