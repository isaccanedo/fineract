...
* Qual nível de registro é apropriado?
  * `LOG.error()` deve ser usado para informar um "operador" executando o Fineract que supervisiona os logs de erro de uma condição inesperada. This includes technical problems with an external "environment" (e.g. can't reach a database), and situations which are likely bugs which need to be fixed in the code.  They do NOT include e.g. validation errors for incoming API requests - that is signaled through the API response - and does (should) not be logged as an error.  (Note that there is no _FATAL_ level in SLF4J; a "FATAL" event should just be logged as an _ERROR_.)
